...
public class ListUtils {
  ...
  public static class ArrangementUtils {
    ...
    public static void rearrageListInZigZag(LinkedList<Integer> list) {
      ...
      while (curr.next != null) {
        ...
        } else if (!smallerThanNext &&
            curr.data < nextNode.data) { // if current is not greater than next, correct the next
          for (target = nextNode; target != null && target.data > curr.data; target = target.next) ;
          if (target == null) {
            target = curr;
          }
  ...
  public static class MergeUtils {
    ...
    static public <T extends Comparable<T>> void mergeInPlaceWithoutFirstListPointerMod(LinkedList<T> list1,
                                                                                        LinkedList<T> list2) {
      if ((list1 == null || list1.start == null) || (list2 == null || list2.start == null)) {
        return;
      }
  ...
  public static class UnionIntersection {
    ...
    private static <T extends Comparable<T>> void intersect(Node<T> start1, Node<T> start2,
